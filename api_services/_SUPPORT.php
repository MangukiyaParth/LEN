...
class CacheHelper
{
	...
	final public static function GetInstance()
	{
		...
		if (IS_PRODUCTION == true) {
			...
			$environment = IS_PRODUCTION == true ? "production" : "sandbox";
...
class SUPPORT
{
	...
	public function call_service($parameter = array())
	{
		...
		try {
			...
			curl_setopt($curl_handle, CURLOPT_URL, API_URL . "manage.php");
	...
	public function UploadImage($file, $thumb_needed, $prepend = "")
	{
		...
		if (isset($_FILES[$file]['name']) && $_FILES[$file]['name'] != '') {
			...
			if ($success) {
				$img_path = API_URL . UPLOAD . 'tmp/' . $_FILES[$file]['name'];
			...
			if ($success && $thumb_needed == true) {
				...
				$img_path = API_URL . UPLOAD . 'tmp_thumb/' . $_FILES[$file]['name'];
	...
	function getjwt($user_id,$issuer,$expmin=90)
	{
		global $secret_key;
		$resp=array();  
		$status=0;
